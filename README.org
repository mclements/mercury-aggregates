...
This is an exploratory exercise, so that the API is likely to change. In particular, the current implementations for =bag_row_number= and =bag_cum_sum= are experimental.
